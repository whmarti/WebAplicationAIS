       ...
       <tr>  
        <td>  
            ...
            <asp:Button ID="Button2" runat="server" Font-Bold="true" Text="Previous" onclick="Button2_Click"   
             ...
             <%if (nCarItems > 0) { %> <span style="float:right;">  <button type="button" onclick="buy('<%=addrCliente %>')" class="btn btn-warning btn-sm mr-1 my-1 " data-idp='<%#Eval("IdProduct")%>'><i class="fa fa-credit-card"></i> Buy</button></span> <%  } else{ %> You don't have items in the car at the moment.<%  } %> 
