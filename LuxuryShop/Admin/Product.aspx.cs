 ...
 * Page function: Page elaborated to update the products to sell.
 * Company: AIS - NZ
 **/
using Business;
using Entity;
...
public partial class Admin_Product : System.Web.UI.Page
{
    ...
    /// <summary>
    /// Function that invokes the logic layer to to bring the product information.
    /// </summary>
    protected void FillData(int pIdProduct)
    {
        product = new Product();
        prodBL = new ProductBL();
        product = prodBL.getProductBL(pIdProduct,ref mError);
        if (product != null)
        {
            ...
            IdProduct.Value = product.IdProduct.ToString();
            name.Text = product.name.Trim();
            stock.Text = Convert.ToString(product.stock).Trim();
            color.Text = product.color.Trim();
            brand.Text = product.brand.Trim();
            size.Text = product.size.Trim();
            lblphoto.Text= product.photo.Trim();
            lblphotoD.Text = product.photoD.Trim();
            img.Value = product.photo.Trim();
            imgD.Value = product.photoD.Trim();
            price.Text = Convert.ToString(product.price);
            discount.Text = Convert.ToString(product.discount);
            bind_month_ddl();
            ddlmonth.SelectedValue = product.monthDcto.ToString(); 
        ...
        {
            _msg = "<strong> Error! </strong> The product is not available.";
